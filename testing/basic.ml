...
let simpleMuxShape kgraph = 	
	let node = simpleOpNodeWtT kgraph in
	node#setContainer (muxContainer ());
	node

let simpleMuxNode kgraph = 
	let node = simpleMuxShape kgraph in
	node#setHeight 30.0;
...
let functionReset kgraph name inputs outputs = 
	let op = function_node kgraph name inputs outputs in
...
let new_edge ?(mult=false) kgraph source target = 
	...
	edge#addData (Style (create_style (LineWidth (if mult then 1.7 else 1.0))));
	edge#addData (Style (create_style ~on_sel:true (LineWidth (if mult then 2.5 else 1.5 ))));
