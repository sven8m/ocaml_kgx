...
val simpleOpContWtT : ?cycle:bool -> unit -> Rendering.containerRendering
(** Creates a simple rectangular container *)

val simpleOpNodeWtT : ?cycle:bool -> Kgraph.kgraph -> Knode.knode
...
val invisiblePort : ?cycle:bool -> Kgraph.kgraph -> Knode.knode -> Kport.kport
(** [invisiblePort kgraph node] creates a port linked to the [node] that is invisible. *)

val invisibleOutputPort : ?cycle:bool -> Kgraph.kgraph -> Knode.knode -> Kport.kport
(** [invisibleOutputPort kgraph node] creates an invisible port linked to the [node] on the eastern side. *)

val invisibleInputPort : ?cycle:bool -> Kgraph.kgraph -> Knode.knode -> Kport.kport
(** [invisibleInputPort kgraph node] creates an invisible port linked to the [node] on the western side. *)

val invisibleControlPort : ?cycle:bool -> ?ofs:float -> Kgraph.kgraph -> Knode.knode -> Kport.kport
(** [invisibleControlPort kgraph node] creates an invisible port linked to the [node] on the north or south side. *)

val visibleControlPort : ?cycle:bool -> ?ofs:float -> Kgraph.kgraph -> Knode.knode -> Kport.kport
...
val visiblePort : ?cycle:bool -> Kgraph.kgraph -> Knode.knode -> Kport.kport
(** [visiblePort kgraph node] creates a port linked to the [node] that is visible (5*5 black box) *)

val visibleOutputPort : ?cycle:bool -> Kgraph.kgraph -> Knode.knode -> Kport.kport
(** [visibleOutputPort kgraph node] creates a visible port linked to the [node] on the eastern side. *)

val visibleInputPort : ?cycle:bool -> Kgraph.kgraph -> Knode.knode -> Kport.kport
(** [visibleInputPort kgraph node] creates a visible port linked to the [node] on the western side. *)

val notOutputPort : ?cycle:bool -> Kgraph.kgraph -> Knode.knode -> Kport.kport
...
val simpleXorNode : ?cycle:bool -> Kgraph.kgraph -> Knode.knode
(** [simpleXorNode kgraph] creates a rectangle with text [=1]*)

val simpleAndNode :  ?cycle:bool -> Kgraph.kgraph -> Knode.knode
(** [simpleAndNode kgraph] creates a rectangle with text [&] *)

val simpleOrNode : ?cycle:bool -> Kgraph.kgraph -> Knode.knode
(** [simpleOrNode kgraph] creates a rectangle with text [>=1] *)

val simpleSelectNode : ?cycle:bool -> Kgraph.kgraph -> string -> Knode.knode
(** [simpleSelectNode kgraph i] creates a rectangle with text [i]. *)

val simpleSliceNode : ?cycle:bool -> Kgraph.kgraph -> string -> string -> Knode.knode
(** [simpleSliceNode kgraph i j] creates a rectangle with text [i..j] .*)

val simpleConcatNode : ?cycle:bool -> Kgraph.kgraph -> Knode.knode
...
val simpleNandNode : ?cycle:bool -> Kgraph.kgraph -> Knode.knode
...
val simpleRegNode : ?cycle:bool -> Kgraph.kgraph -> Knode.knode
(** [simpleRegNode kgraph] creates a rectangle with a triangle at the bottom. *)

val simpleBufferNode : ?cycle:bool -> Kgraph.kgraph -> Knode.knode
...
val simpleMuxShape : ?cycle:bool -> unit -> Rendering.containerRendering

(** Creates a container Rendering that has the shape of a mux *)

val simpleMuxNode : ?cycle:bool -> Kgraph.kgraph -> Knode.knode
(** [simpleMuxNode kgraph] creates an node with mux shape *)

val simpleCondNode : ?cycle:bool -> Kgraph.kgraph -> Knode.knode
(** [simpleCondNode kgraph] creates a node with mux shape *)

val simpleMatchNode : ?cycle:bool -> Kgraph.kgraph -> Knode.knode
...
val simpleFbyNode : ?cycle:bool -> Kgraph.kgraph -> Knode.knode
...
val function_node : ?cycle:bool -> ?res:bool -> ?aut:bool -> ?m:bool -> Kgraph.kgraph -> string -> int -> Knode.knode
...
val stateNode : ?cycle:bool -> ?init:bool -> Kgraph.kgraph -> string -> int -> Knode.knode
...
val simpleSyncNode : ?cycle:bool -> ?init:bool -> Kgraph.kgraph -> Knode.knode
(** [simpleSyncNode kgraph] creates a node for sync parts.
	Option [init] indicates if the node is initial. (default [false])*)

val simpleSeqBlockNode : ?cycle:bool -> Kgraph.kgraph -> string -> Knode.knode
...
val ramNode : ?cycle:bool -> Kgraph.kgraph -> Knode.knode
(** [ramNode kgraph] creates a ram node with title [ram]. *)

val romNode : ?cycle:bool ->  Kgraph.kgraph -> Knode.knode
(** [romNode kgraph] creates a rectangular node with title [rom]. *)	

val junction : ?cycle:bool ->  bool -> Rendering.containerRendering
(** [junction mult] creates a junction buble of size 4, 5 if [mult] is true (default [false])*)

val arrow_decorator : ?cycle:bool -> ?tiny:bool -> ?h:bool -> bool -> Rendering.containerRendering
...
val new_edge : ?cycle:bool -> ?mult:bool -> Kgraph.kgraph -> Knode.knode -> Kport.kport -> Knode.knode -> Kport.kport -> Intermediate_graph.edge_label list -> unit
...
val linkEdge : ?cycle:bool -> Kgraph.kgraph -> Knode.knode -> Kport.kport -> Knode.knode -> Kport.kport -> unit
(** [linkEdge kgraph sourceNode sourcePort targetNode targetPort] creates a green polyline from [sourceNode] with port [sourcePort] to [targetNode] with port [targetPort],
*)
